 ...
 * @Description:    评论消息详情页
 * @Author:         张璐婷
 * @CreateDate:     2020/4/25 14:58
 * @Version:        1.0
 */
public class MailCommentDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(mailMyComment.getCrFlag() == 'C'){
            ...
            if(note.getImgList().size() > 0) {
                Glide.with(this).load("http://" + ip + ":8080/MoJi/"+note.getImgList().get(0)).into(noteImg);
            }else{
                noteImg.setImageResource(R.mipmap.default_bg);
            }
            noteContent.setText(note.getContent());
            noteUserName.setText(note.getUser().getUserName());
            ...
            note = replyComment.getComment().getNote();
            if(note.getImgList().size() > 0) {
                Glide.with(this).load("http://" + ip + ":8080/MoJi/"+note.getImgList().get(0)).into(noteImg);
            }else{
                noteImg.setImageResource(R.mipmap.default_bg);
        ...
        jumpDropsDetail.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent1 = new Intent(getApplicationContext(),DropsDetailActivity.class);
